...
const Form = () => {
  ...
  const checkCodeInput = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    codeInput === code ? setCodeValid("correct") : setCodeValid("wrong");
  };
  const regenerateCode = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
  ...
  return (
    <form onSubmit={checkCodeInput}>
      ...
      <OtpInput
        isInputNum={true}
        value={codeInput}
        onChange={(enteredCode: any) => setCodeInput(enteredCode)}
        numInputs={6}
        isSuccessed={true}
        separateAfter={3}
        shouldAutoFocus
